...
async function updateProfile(payloadData, userData) {
    ...
    // if (payloadData.currentLocation!==undefined){
    //     if (JSON.parse(payloadData.currentLocation).length) dataToUpdate.currentLocation = JSON.parse(payloadData.currentLocation);
    // }
...
async function addEditVCs(payloadData, userData) {
    ...
    if (payloadData.description) dataToUpdate.description = payloadData.description;
    if (payloadData.investmentFocus) dataToUpdate.investmentFocus = payloadData.investmentFocus;
...
async function addEditAccelerator(payloadData, userData) {
    ...
    if (payloadData.requirements) dataToUpdate.requirements = payloadData.requirements;
    if (payloadData.length) dataToUpdate.length = JSON.parse(payloadData.length);
...
async function exploreAccelerator(payloadData, userData)  {
    ...
    if (payloadData.length!==undefined){
        if (JSON.parse(payloadData.length).length) criteria.length = {$in: JSON.parse(payloadData.length)};
    }
...
async function searchUser(payloadData) {
    ...
    let getResult = await getRequired(Modal.Users,criteria,{_id:1,fullName:1,email:1},{lean:true,sort:{fullName:1}});
    return getResult
}
